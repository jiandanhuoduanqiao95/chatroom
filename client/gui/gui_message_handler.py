...
class MessageHandler:
            ...
            if msg_type == "chat":
                try:
                    ...
                    if sender == "服务器" or sender == "[系统公告]":
                        ...
                        self.client_gui.chat_ui.append_chat(sender, f"{tag}{sender}: {msg} ({message_id})",
                                                            tag=f"clickable_message_{message_id}")
                ...
                self.client_gui.chat_ui.append_chat(sender,
                                                    f"{tag}{sender}: 收到文件: {filename}，已保存至 {file_path} ({message_id})",
                                                    tag=f"clickable_message_{message_id}")
        elif msg_type == "error":
        ...
        for target_message_id in target_message_ids:
            ...
            for msg in self.client_gui.chat_histories[friend]:
                if msg.get('tag', '') == f"clickable_message_{target_message_id}":
                    msg['text'] = f"{sender or self.client_gui.username}: [消息已撤回] ({target_message_id})\n"
                    break

            logging.info(f"历史记录更新: 消息ID={target_message_id}, 撤回 在 {friend}")
            ...
            if friend == self.client_gui.current_friend and friend in self.client_gui.chat_windows:
                ...
                chat_text.see('end')
                logging.info(f"当前聊天窗口更新: 消息ID={target_message_id}, 撤回")
    ...
    def send_chat(self):
        ...
        try:
            if self.client_gui.current_friend.startswith("群组 "):
                ...
                self.client_gui.chat_ui.append_chat(self.client_gui.current_friend,
                                                    f"{self.client_gui.username}: {msg} ({message_id})",
                                                    tag=f"clickable_message_{message_id}")
    ...
    def send_file(self):
        ...
        try:
            ...
            self.client_gui.chat_ui.append_chat(self.client_gui.current_friend,
                                                f"已发送文件: {filename} ({filesize} bytes) ({message_id})",
                                                tag=f"clickable_message_{message_id}")
        except Exception as e:
    ...
    def _update_friend_list(self, friends, action_result=None):
        """在主线程中安全更新好友列表 Treeview（不显示在线状态，仅清除好友条目，保留群组）"""
